...
export async function virtual(): Promise<Plugin> {
	const virtualModuleIds: string[] = ["virtual:eps", "virtual:svg-register", "virtual:svg-icons"];

	return {
		name: "vite-flyway-virtual",
		enforce: "pre",
		...
		handleHotUpdate({ file, server }) {
			// 文件修改时触发
			if (
				!["pages.json", "dist", "build/flyway", "eps.json", "eps.d.ts"].some((e) =>
					file.includes(e),
				)
			) {
				createEps().then((data) => {
